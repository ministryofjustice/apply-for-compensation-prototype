module.exports = function (router, content) {
  ...
  router.post('/concepts/physical-injuries/one-q-page-v4/injured-body-parts/legs', function (req, res) {
    let legInjuredParts = req.session.data['legInjuredParts'] || []
    let injuredParts = req.session.data['injuredParts'] || []

    if (legInjuredParts.includes('Hip')) {
      res.redirect('/concepts/physical-injuries/one-q-page-v4/injured-body-parts/legs/hip')
    } else if (legInjuredParts.includes('Leg')) {
      res.redirect('/concepts/physical-injuries/one-q-page-v4/injured-body-parts/legs/leg')
    } else if (legInjuredParts.includes('Knee')) {
      res.redirect('/concepts/physical-injuries/one-q-page-v4/injured-body-parts/legs/knee')
